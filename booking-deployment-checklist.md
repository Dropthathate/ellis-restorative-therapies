...
Booking requests include the selected therapist. Hunter requests route to `hunterellissss@gmail.com` and Zachary is copied on those notices. Zachary requests route to `restorewithellis@gmail.com`. Client confirmations identify the selected therapist.
...
2. Share Hunter’s Google Calendar with `restorewithellis@gmail.com`, allowing that account to **see event details**. The Apps Script runs as this account and uses calendar events containing `[ERT]` to hide booked times.
