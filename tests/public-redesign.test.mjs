import assert from 'node:assert/strict';
import { existsSync, readFileSync } from 'node:fs';

const index = readFileSync(new URL('../index.html', import.meta.url), 'utf8');
const css = readFileSync(new URL('../public-redesign.css', import.meta.url), 'utf8');
const booking = readFileSync(new URL('../book.html', import.meta.url), 'utf8');
const robots = readFileSync(new URL('../robots.txt', import.meta.url), 'utf8');
const sitemap = readFileSync(new URL('../sitemap.xml', import.meta.url), 'utf8');
const llms = readFileSync(new URL('../llms.txt', import.meta.url), 'utf8');

assert.equal(existsSync(new URL('../favicon.ico', import.meta.url)), true);

assert.match(index, /Zachary Ellis/);
assert.match(index, /Hunter Ellis/);
assert.match(index, /class="hero-background-logo"/);
assert.match(index, /class="background-logo-halo"/);
assert.doesNotMatch(index, /class="hero-logo-stage/);
assert.match(index, /brand-mark brand-mark-logo/);
assert.match(index, /CAMTC #97101/);
assert.match(index, /CAMTC #103413/);
assert.match(index, /book\.html\?therapist=zachary/);
assert.match(index, /book\.html\?therapist=hunter/);
assert.match(index, /https:\/\/client\.restorewithellis\.com/);
assert.match(index, /src="zachary\.png"/);
assert.match(index, /src="hunter-ellis\.jpg"/);
assert.doesNotMatch(index, /manus-storage|ellisdash-jvnvllzx\.manus\.space/);
assert.doesNotMatch(index, /Google Reviewer|aggregateRating/i);
assert.match(index, /Therapeutic & Neuromuscular Massage in Modesto, CA/);
assert.match(index, /2209 Coffee Rd, Suite M/);
assert.match(index, /Neuromuscular Massage in Modesto/);
assert.match(index, /ellisrestorativetherapies/);
assert.match(index, /facebook\.com\/p\/Ellis-Restorative-Therapies/);
assert.match(css, /\.therapist-grid/);
assert.match(css, /\.therapist-card/);
assert.match(css, /--ink:#cad5d3/);
assert.match(css, /--teal:#16717c/);
assert.match(css, /--gold:#963641/);
assert.equal(existsSync(new URL('../camtc-logo-official.jpg', import.meta.url)), false);
assert.equal(existsSync(new URL('../amta-california-chapter-member-logo.png', import.meta.url)), true);
assert.match(index, /Official Ellis Restorative Therapies logo/);
assert.match(index, /California Massage Therapy Council certification/);
assert.doesNotMatch(index, /camtc-logo-official\.jpg/);
assert.match(index, /American Massage Therapy Association California Chapter member logo/);
assert.match(index, /AMTA California Chapter member/);
assert.match(index, /HSA\/FSA information/);
assert.match(index, /Payment eligibility and processing can vary/);
assert.doesNotMatch(index, /HSA\/FSA[^<]{0,80}(?:accepted|we accept|acceptance|eligible|covered)/i);
assert.match(index, /Before your session/);
assert.match(index, /Powered by SOMASYNC AI/);
assert.match(index, /AALIYAH/);
assert.match(css, /\.brand-trust-section/);
assert.match(css, /\.prepare-section/);
assert.match(css, /\.systems-credit/);
assert.match(css, /\.credential-text-mark/);
assert.match(css, /\.hero-fiber-field/);
assert.match(css, /\.logo-mount/);
assert.match(css, /\.hero-background-logo/);
assert.match(css, /@keyframes backgroundMarkBreathe/);
assert.match(css, /opacity:\.15/);
assert.match(css, /@keyframes fiberBreathe/);
assert.match(css, /prefers-reduced-motion:no-preference/);
assert.match(booking, /new URLSearchParams\(window\.location\.search\)\.get\('therapist'\)/);
assert.match(booking, /THERAPISTS\[requestedTherapist\]/);
assert.match(booking, /Book Therapeutic Massage in Modesto, CA/);
assert.match(booking, /https:\/\/www\.restorewithellis\.com\/book\.html/);
assert.match(robots, /Sitemap: https:\/\/www\.restorewithellis\.com\/sitemap\.xml/);
assert.match(sitemap, /https:\/\/www\.restorewithellis\.com\/services\.html/);
assert.match(sitemap, /https:\/\/www\.restorewithellis\.com\/book\.html/);
assert.match(llms, /neuromuscular massage/i);
assert.match(llms, /2209 Coffee Rd, Suite M, Modesto, CA 95355/);

console.log('Public redesign and therapist-link assertions passed.');
